<template>
  <div>
    <RecruitmentDetail v-if="recruitmentDetail" :title="'Recruitment'" :is-view="true" />
    <b-alert class="mt-3" :show="showError" variant="danger">{{ responseError }}</b-alert>
  </div>
</template>
<script lang="ts">
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      if (this.recruitmentID) {
        ...
        this.$router.push('/manage-recruitment');
      }
    });
  }
